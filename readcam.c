...
if (fd>0) printf("Open video1\n");
}
  else
     printf("Successfull!\n");

// get the capability of the video
  if (ioctl(fd,VIDIOC_QUERYCAP,& cap)==-1)  printf("Can't get cap!\n");
  if (cap.capabilities&V4L2_CAP_VIDEO_CAPTURE) printf("It is a video caprure device!\n");
  if (cap.capabilities&V4L2_CAP_STREAMING) printf("It support streaming i/0\n");
  if (cap.capabilities&V4L2_CAP_READWRITE) printf("It support read and write i/o\n");
// set up the image format
   fmt.type=V4L2_BUF_TYPE_VIDEO_CAPTURE;
   fmt.fmt.pix.width=COLS;
   fmt.fmt.pix.height=ROWS;
   fmt.fmt.pix.pixelformat=V4L2_PIX_FMT_YUYV;
if ( ioctl(fd,VIDIOC_S_FMT,& fmt)==-1) printf ( "format failed\n");
    else printf(" This format is allowed!\n");
// set up the numbers of driver buffer
   reqbuf.type=V4L2_BUF_TYPE_VIDEO_CAPTURE;
   reqbuf.count=30;
   reqbuf.memory=V4L2_MEMORY_MMAP;
if (ioctl(fd,VIDIOC_REQBUFS,& reqbuf)==-1) printf ("Buffer request error\n"); 

//allocate the approcation buffers 
buffers = calloc (reqbuf.count, sizeof (*buffers));
if(!buffers) printf("Buffers out of memory\n");
